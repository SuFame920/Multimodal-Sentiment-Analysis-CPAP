...
class MOSI:
    def __init__(self, config):
        ...
        try:
            if config.args.iid_setting:
                DATA_PATH=DATA_PATH+"/split_dataset_2" if not config.args.seven_class else DATA_PATH+"/split_dataset_7classes_1"
                self.train = load_pickle(DATA_PATH + '/train.pkl')
                self.dev = load_pickle(DATA_PATH + '/dev.pkl')
                self.test = load_pickle(DATA_PATH + '/test_IID.pkl')
                self.pretrained_emb, self.word2id = None, None
            elif config.args.ood_setting:
                DATA_PATH=DATA_PATH+"/split_dataset_2" if not config.args.seven_class else DATA_PATH+"/split_dataset_7classes_1"
                self.train = load_pickle(DATA_PATH + '/train.pkl')
                self.dev = load_pickle(DATA_PATH + '/dev.pkl')
                self.test = load_pickle(DATA_PATH + '/test_OOD.pkl')
                self.pretrained_emb, self.word2id = None, None
            else:
                self.train = load_pickle(DATA_PATH + '/train.pkl')
                self.dev = load_pickle(DATA_PATH + '/dev.pkl')
            ...
            if True:
                ...
                label = np.concatenate(
                    ...
                    (train_split_noalign['regression_labels'], dev_split_noalign['regression_labels'], test_split_noalign['regression_labels']), axis=0)
            ...
            for i, idd in enumerate(all_id_list):
                ...
                ###
                idd1 = idd1.strip().strip('$')
                idd2 = idd2.strip().strip('$')

                #print("当前待匹配的 id:", (idd1, idd2))
                #print("部分 all_csv_id 数据:", all_csv_id[:10])
                ...
                _label = label[i].reshape(-1,1).astype(np.float32)
                _visual = v[i]
                _acoustic = a[i]
                _vlen = vlens[i]
                _alen = alens[i]
                ...
                visual = _visual[:_vlen, :]
                acoustic = _acoustic[:_alen, :]
